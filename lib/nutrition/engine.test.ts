...
test('goals - cut/recomp/lean_bulk/maintain calorie targets scale off TDEE', () => {
  const tdee = calculateTDEE(calculateBMR('male', 75, 175, 28), 'moderately_active')
...
test('Part 18 worked example: male/28/75kg/175cm/moderately_active/cut', () => {
  const target = buildNutritionTarget(
    baseProfile({ sex: 'male', age: 28, weightKg: 75, heightCm: 175, activityLevel: 'moderately_active', goal: 'cut' })
  )
  // ~2650 estimated maintenance, ~2250 cut target, 150g protein, 60g fat, ~278g carbs.
  assert.ok(Math.abs(target.estimatedMaintenanceCalories - 2650) <= 10)
  assert.ok(Math.abs(target.calories - 2250) <= 10)
  assert.strictEqual(target.protein, 150)
  assert.strictEqual(target.fat, 60)
  assert.ok(Math.abs(target.carbs - 278) <= 5)
})
